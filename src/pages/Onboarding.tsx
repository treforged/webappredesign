...
import {
  ...
  ChevronLeft, Check, Crown, Zap, BarChart3, Shield, Loader2,
} from 'lucide-react';
...
interface DebtEntry { name: string; balance: string; apr: string; minPayment: string; }
interface GoalEntry  { name: string; targetAmount: string; }
    ...
    <div className="space-y-2">
      <div className="flex justify-between text-[10px] text-muted-foreground">
        {STEPS.slice(0, -1).map((s, i) => (
          <span key={s} className={i <= idx ? 'text-primary font-medium' : ''}>{STEP_LABELS[s]}</span>
        ))}
      </div>
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className={`w-full bg-secondary border border-border py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring ${prefix ? 'pl-7 pr-3' : 'px-3'}`}
...
export default function Onboarding() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // 1. Update profile (profile is auto-created on signup)
      ...
      // 2. Create expenses as budget items (column is `label` not `name`)
      const expenses = [
      ...
      // 3. Create debt accounts (column is `min_payment` not `minimum_payment`)
      if (data.debts.length > 0) {
        await supabase.from('debts').insert(
          data.debts
            .filter(d => d.name && parseFloat(d.balance) > 0)
            .map(d => ({
              user_id: user!.id,
              name: d.name,
      ...
      if (data.goals.length > 0) {
        await supabase.from('savings_goals').insert(
          data.goals
            .filter(g => g.name && parseFloat(g.targetAmount) > 0)
            .map(g => ({
              user_id: user!.id,
              name: g.name,
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md space-y-6">
        ...
        {/* Progress */}
        {step !== 'finish' && <ProgressBar step={step} />}

        {/* Card */}
        <div className="card-forged p-6 space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <FieldLabel>Weekly Gross ($)</FieldLabel>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={e => update('paycheckFrequency', e.target.value)}
          ...
          {step === 'expenses' && (
            <div className="space-y-4">
              ...
              <p className="text-[10px] text-muted-foreground">Approximate is fine — you can adjust later.</p>
              {[
          ...
          {step === 'debts' && (
            <div className="space-y-4">
              ...
              {data.debts.map((d, i) => (
                <div key={i} className="space-y-2 p-3 bg-secondary/40 border border-border" style={{ borderRadius: 'var(--radius)' }}>
                  ...
                  <Input value={d.name} onChange={v => update('debts', data.debts.map((dd, j) => j === i ? { ...dd, name: v } : dd))} placeholder="Card / loan name" />
                  <div className="grid grid-cols-3 gap-2">
                    <div className="space-y-1">
                      <span className="text-[9px] text-muted-foreground uppercase">Balance</span>
                      <Input value={d.balance} onChange={v => update('debts', data.debts.map((dd, j) => j === i ? { ...dd, balance: v } : dd))} placeholder="0" type="number" prefix="$" />
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] text-muted-foreground uppercase">APR %</span>
                      <Input value={d.apr} onChange={v => update('debts', data.debts.map((dd, j) => j === i ? { ...dd, apr: v } : dd))} placeholder="0" type="number" />
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] text-muted-foreground uppercase">Min. Pay</span>
                      <Input value={d.minPayment} onChange={v => update('debts', data.debts.map((dd, j) => j === i ? { ...dd, minPayment: v } : dd))} placeholder="0" type="number" prefix="$" />
                    </div>
                  </div>
                </div>
              ))}
              <button
                onClick={() => update('debts', [...data.debts, { name: '', balance: '', apr: '', minPayment: '' }])}
                className="w-full py-2 text-xs font-medium border border-dashed border-border text-muted-foreground hover:border-primary/40 hover:text-primary transition-colors"
                style={{ borderRadius: 'var(--radius)' }}
              >
                + Add a debt
              </button>
            ...
            <div className="space-y-4">
              ...
              {data.goals.map((g, i) => (
                <div key={i} className="space-y-2 p-3 bg-secondary/40 border border-border" style={{ borderRadius: 'var(--radius)' }}>
                  ...
                  <Input value={g.name} onChange={v => update('goals', data.goals.map((gg, j) => j === i ? { ...gg, name: v } : gg))} placeholder="e.g. Emergency Fund, New Car, Vacation" />
                  <div className="space-y-1">
                    <span className="text-[9px] text-muted-foreground uppercase">Target amount</span>
                    <Input value={g.targetAmount} onChange={v => update('goals', data.goals.map((gg, j) => j === i ? { ...gg, targetAmount: v } : gg))} placeholder="0" type="number" prefix="$" />
                  </div>
                </div>
              ))}
              {data.goals.length < 3 && (
                <button
                  onClick={() => update('goals', [...data.goals, { name: '', targetAmount: '' }])}
                  className="w-full py-2 text-xs font-medium border border-dashed border-border text-muted-foreground hover:border-primary/40 hover:text-primary transition-colors"
                  style={{ borderRadius: 'var(--radius)' }}
                >
                  + Add a goal
          ...
          {/* ── Finish / Insights + Premium upsell ──────────────────────── */}
          {step === 'finish' && (
            <div className="space-y-5">
              <div className="text-center space-y-2">
                <div className="w-12 h-12 bg-primary/15 border border-primary/30 rounded-full flex items-center justify-center mx-auto">
              ...
              <div className="border border-primary/25 bg-primary/5 p-4 space-y-3" style={{ borderRadius: 'var(--radius)' }}>
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px]">
                  {[
                    ['✓ Auto-sync transactions', '✓ Plaid bank connection'],
                    ['✓ Unlimited history', '✓ Priority support'],
                  ].flat().map(f => (
                    <div key={f} className="flex items-center gap-1 text-muted-foreground">
                      <Shield size={9} className="text-primary shrink-0" /> {f.replace('✓ ', '')}
                    </div>
                  ))}
                </div>
                <div className="flex gap-2 pt-1">
                  <a
                    ...
                    className="flex-1 text-center py-1.5 text-[10px] font-semibold bg-primary text-primary-foreground btn-press"
                    style={{ borderRadius: 'var(--radius)' }}
                  >
                    Explore Premium
                  </a>
                  <button
                    ...
                    disabled={saving}
                    className="flex-1 py-1.5 text-[10px] font-medium border border-border text-muted-foreground hover:text-foreground btn-press disabled:opacity-50"
                    style={{ borderRadius: 'var(--radius)' }}
                  >
          ...
          {step !== 'finish' && (
            <div className="flex items-center justify-between pt-2">
              ...
              <button
                onClick={step === 'goals' ? next : next}
                className="flex items-center gap-1.5 bg-primary text-primary-foreground px-4 py-2 text-xs font-semibold btn-press"
                style={{ borderRadius: 'var(--radius)' }}
              >
